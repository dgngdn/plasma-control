...
// Slews the MCP4261 over its operating range and reads the output on A0.
// For best results, buffer the potentiometer output from the Arduino input.
// This example demonstrates control over SPI to the Microchip McpDigitalPot Digital potentiometer
...
// Including SPI.h vv below initializes the MOSI, MISO, and SPI_CLK pins as per ATMEGA 328P
#include <SPI.h>
...
void setup()
{
  ...
  digitalPot.scale = 100; // For the timeout example, below
...
// Cycle the wipers around at 10% increments, changing every 1 second
