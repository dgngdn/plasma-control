...
// wrappers for the d motor!
void forwardstep_d() {  
  dMotor->onestep(FORWARD, DOUBLE);
}
void backwardstep_d() {  
  dMotor->onestep(BACKWARD, DOUBLE);
}

// wrappers for the x motor!
void forwardstep_x() {  
...
// wrappers for the y motor!
void forwardstep_y() {  
  yMotor->onestep(FORWARD, SINGLE);
}
void backwardstep_y() {  
  yMotor->onestep(BACKWARD, SINGLE);
}

AccelStepper stepperx(forwardstep_x, backwardstep_x);
AccelStepper steppery(forwardstep_y, backwardstep_y);
AccelStepper stepperd(forwardstep_d, backwardstep_d);
...
void manual_input(String input) {
  ...
  switch( input.charAt(0) ) {
    ...
    case 'q' :
      ...
      case 'd' :
      // you sent d,###
      if (input.substring(2).toFloat() > 15) {setpoint::dist=15;}
      else if (input.substring(2).toFloat() < -4) {setpoint::dist=-40;}
      else {
...
//  yMotor->setSpeed(30);  // 10 rpm   
  ...
  stepperd.setMaxSpeed(400.0);
  stepperd.setAcceleration(1000.0);
...
void loop()
{
  ...
  long positions[3]; // Array of desired stepper positions
        ...
        positions[0]=setpoint::dist*10;
        positions[1]=setpoint::dist*10;
        positions[2]=setpoint::dist*10;
 ...
 // mystring += ',';
  mystring += stepperd.currentPosition()/10.0;
  mystring += ',';
  ...
  mystring += data::i_rms;
  mystring += ',';
  mystring += stepperx.currentPosition();
  mystring += ',';
  mystring += steppery.currentPosition();
