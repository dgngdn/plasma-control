...
void actuate_inputs() {
  ...
  setpoint::vapp=setpoint::vapp + PI_V::Kp*(PI_V::err+PI_V::I/PI_V::Tau_i);  

  DAC_FXN.Set(mapfloat(setpoint::vapp,0,10,0.96*(DACSTEPS-1),0.55*(DACSTEPS-1)), 
...
int move_to_pos(float delt, Adafruit_StepperMotor *motor) {
  ...
  if (delt < 0) {
      myMotor->step(24, FORWARD, DOUBLE); 
      delt=delt + 2;
      //delay(30);
     return delt;
  } else if (delt > 0) {
      delt=delt - 2;
      myMotor->step(24, BACKWARD, DOUBLE); 
      //delay(30);
    return delt;
  } else if (delt == 0) {
    return delt;
...
void loop()
{
  ...
  PI_V::err = 0.76*setpoint::voltage - data::v_rms;
  actuate_inputs();
  PI_V::I = PI_V::I + PI_V::err*12.5e-3;
