...
Lt, Pt = core.dlqe(A,C,NP.eye(nx),0.1*NP.eye(ny))
...
if __name__ == "__main__":
  ...
  while True:
    ...
    if startMPC:
      ...
      send_inputs(f,U)
      print("inputs sent!")
      print("predicted temperature: {:.2f} intensity: {:.2f}".format(*(C.dot(pred).T+Y0).flatten()))
      ...
      save_file.write("{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f},{:6.2f}\n".format(time.time(),Ts,Is,*(C.dot(pred).T+Y0).flatten(),*Y,*U.flatten(),*ztar_k,time_el))  ##X is never referenced!
      #print()
      save_file.flush()
      if time_el < Delta:
